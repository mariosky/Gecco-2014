...
the natural search process described by the biological theory of Neo-Darwinian evolution \cite{}. 
Inspired in biological evolution, EC researchers have developed a variety of search and optimization algorithms \cite{}.
...
which are mostly coded as sequential and synchronous algorithms \cite{}.
...
For instance, Fern\'andez et al. \cite{}% articulo Paco, Gustavo y Leo publucado en Natural Computing}
...
This work, however, focuses on EvoSpace, a framework to develop PEAs using heterogeneous and unreliable resources \cite{}, described in greater
...
However, recent work in \cite{} has shown a promising alternate approach for EAs that employ multiple evolving populations.
In particular, the RPSS approach proposed in \cite{} is as interesting as it is simple.
First, consider the original configuration studied in \cite{}, an Island Model EA.
...
Therefore, the proposal in \cite{} is to set the parameter values randomly, without a tuning or self-adaptive process whatsoever.
...
Nevertheless, results reported in \cite{} exhibit promise, achieving competitive results while substantially reducing the amount
...
Following \cite{}, the proposal of the current work is to apply the RPSS approach to a PEA developed with EvoSpace.
However, before turning to the experimental work, lets highlight the main differences between a PEA and the Island Model studied in \cite{}.
